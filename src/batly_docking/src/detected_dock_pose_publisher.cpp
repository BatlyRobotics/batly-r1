...
public:
  ...
  : Node("detected_dock_pose_publisher")
  {
    ...
    this->declare_parameter("parent_frame", "camera_optical_link");
 ...
 void timer_callback()
{
  ...
  try {
    ...
    auto transform_age = this->get_clock()->now() -
      rclcpp::Time(transform.header.stamp);
    if (transform_age > rclcpp::Duration::from_seconds(0.5)) {
      RCLCPP_WARN(this->get_logger(), "Transform is stale (%.2fs old), skipping",
        transform_age.seconds());
      return;
    }
